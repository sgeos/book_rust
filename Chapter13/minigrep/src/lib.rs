...
impl Config {
  pub fn new(mut args: env::Args) -> Result<Config, (String, &'static str)> {
    let executable = match args.next() {
      Some(arg) => arg,
      None => return Err(("minigrep".to_string(), "not enough arguments, missing executable")),
    };
    let query = match args.next() {
      Some(arg) => arg,
      None => return Err((executable, "No query string.")),
    };
    let filename = match args.next() {
      Some(arg) => arg,
      None => return Err((executable, "No filename.")),
    };
    let mut case_sensitive = env::var("CASE_INSENSITIVE").is_err();
    for arg in args {
      case_sensitive = match arg.as_ref() {
